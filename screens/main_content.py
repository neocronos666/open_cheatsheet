...
class MainContent(ft.Column):
    ...
    def load_directory_structure(self):
        ...
        return directory_structure    # !!!!!!!!!!!!!!!!ACA ESTA EL PROBLEMA; SE CAMBIO ESTA ESTRUCTURA!!!!!!!!!!!!!!!!!!!!!
        ...
        for directory, subdirectories in structure.items():
            directory_view.controls.append(
                ft.Container(
                    ...
                    # Usa functools.partial para pasar el directorio como parámetro
    ...
    def build_directory_view(self, structure=None, level=0):
        ...
        if isinstance(structure, dict):  # Verificamos si structure es un diccionario
            for directory, subdirectories in structure.items():
                ...
                # file_count = self.count_files_in_directory(subdirectories)
                # file_count = self.directory_structure.get("__file_count__", 0)
    ...
    def count_files_in_directory(self, subdirectories):
        ...
        if len(subdirectories):
            ...
            for subdir, contents in subdirectories.items():
                # Si no hay subdirectorios, cuenta como archivo
                if not contents:
                    count += 1
                else:
                    count += self.count_files_in_directory(contents)
            '''
            for subdir, contents in subdirectories.items():
...
class MainContent(ft.Column):
    ...
    def on_category_click(self, e): # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!ROTO!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!        
